...
""" 
Modified Depth Refinement Network (DRN) refines initial depth map from COLMAP and provides uncertainty estimates
- Inputs: 
        (1) rgb image - image; 
        (2) surface normal - normal;
        (3) triangulated depth - depth_flow; 
...
class DRNModified(ExtendedFPN):
    def __init__(self, arguments):
        super().__init__(arguments)
        self.args = arguments
        self.feature_concat = None

        self.resnet_depth = ResNetPyramids(in_channels=2, pretrained=False, resnet_arch=self.args.resnet_arch)
        ...
        self.update_block = UpdateBlockIRM(
            hidden_dim = self.input_channels[0] // 2 * self.num_encoders,
            input_dim = 16*9+4-1,
